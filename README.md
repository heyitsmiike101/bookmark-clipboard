...
**Post a text clip and a file to the clipboard (agent-friendly, pure JSON):**
```bash
curl -X POST "http://localhost:3000/api?action=clip" \
  -H "Content-Type: application/json" -d '{"text":"note from an agent"}'

curl -X POST "http://localhost:3000/api?action=add-file" \
  -H "Content-Type: application/json" \
  -d '{"filename":"summary.txt","contentText":"Generated by an agent."}'
```
